      ...
      <div className="header">
        <div className="flex">
          <div className="flex w-10">
            <button className={loadEvents ? "active" : "Inactive"} id="headerButtonInactive" onClick={liveEvents}>Live</button>
            <button className={!loadEvents ? "active" : "Inactive"} id="headerButtonActive" onClick={pauseEvents}>Pause</button>
          </div>
          <div id="searchDiv" className="w-86">
            <img id="searchIcon" src={searchIcon} alt="Search icon" />
            <input type="text" defaultValue={searchKeyword ? searchKeyword : ""} onChange={(event) => { searchEventHandler(event.target.value) }} placeholder="Type to search" />
          </div>
        </div>
      </div>
